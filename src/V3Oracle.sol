...
contract V3Oracle is IV3Oracle, Ownable2Step, Constants {
    ...
    return feedConfigs[token];
}
function getChainlinkPriceX96(address token) external view returns (uint256) {
    return _getChainlinkPriceX96(token);
}

function getTWAPPriceX96(TokenConfig memory config) external view returns (uint256) {
    return _getTWAPPriceX96(config);
}
}
