...
contract AutoCompound is Transformer, Automator, Multicall, ReentrancyGuard {
    ...
    function execute(ExecuteParams calldata params) external nonReentrant {
        ...
        console2.log("Collected amount0:", state.amount0);
        console2.log("Collected amount1:", state.amount1);

        // get position info
        (,, state.token0, state.token1, state.fee, state.tickLower, state.tickUpper,,,,,) =
            nonfungiblePositionManager.positions(params.tokenId);

        console2.log("Token0 address:", state.token0);
        console2.log("Token1 address:", state.token1);
        console2.log("Fee:", state.fee);
        ...
        console2.log("Total amount0:", state.amount0);
        console2.log("Total amount1:", state.amount1);

        // only if there are balances to work with - start autocompounding process
        if (state.amount0 != 0 || state.amount1 != 0) {
            uint256 amountIn = params.amountIn;
            console2.log("Initial amountIn:", amountIn);
            ...
            if (amountIn != 0) {
                ...
                try pool.slot0() returns (
                    ...
                    state.tick = tick;
                    console2.log("SqrtPriceX96:", state.sqrtPriceX96);
                    console2.log("Current tick:", state.tick);
                } catch {
                    revert("Failed to get pool state with slot0()");
                }
                ...
                if (tSecs != 0) {
                    if (!_hasMaxTWAPTickDifference(pool, tSecs, state.tick, maxTWAPTickDifference)) {
                        ...
                        amountIn = 0;
                        console2.log("TWAP check failed, amountIn set to 0");
                    }
                }
                ...
                if (amountIn != 0) {
                    ...
                    console2.log("amountInDelta:", state.amountInDelta);
                    console2.log("amountOutDelta:", state.amountOutDelta);
                }
            }
            ...
            console2.log("Max add amount0:", state.maxAddAmount0);
            console2.log("Max add amount1:", state.maxAddAmount1);
