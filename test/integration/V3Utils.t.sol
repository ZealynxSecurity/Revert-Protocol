...
contract V3UtilsIntegrationTest is Test {
    ...
    function testUnauthorizedTransfer() external { //@audit
    ...
    function testInvalidInstructions() external { //@audit 
    ...
    function testSendEtherNotAllowed() external { //@audit
        bool success;
        vm.expectRevert(Constants.NotWETH.selector);
        (success,) = address(v3utils).call{value: 123}("");
    }

    function testTransferDecreaseSlippageError() external { //@audit --via-ir
    ...
    function testTransferAmountError() external { //@audit --via-ir
        // add liquidity to existing (empty) position (add 1 DAI / 0 USDC)
        _increaseLiquidity();
    ...
    function testTransferWithChangeRange() external { //@audit --via-ir
        // add liquidity to existing (empty) position (add 1 DAI / 0 USDC)
        _increaseLiquidity();
    ...
    function testFailEmptySwapAndIncreaseLiquidity() external {  //@audit --via-ir
