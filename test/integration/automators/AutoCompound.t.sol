...
contract AutoCompoundTest is AutomatorIntegrationTestBase {
    ...
    function testTWAPVerificationWithZeroAmountIn() external {
        ...
        // Perform the execute operation with amountIn set to 0
        vm.prank(OPERATOR_ACCOUNT);
        try autoCompound.execute(AutoCompound.ExecuteParams(TEST_NFT_2, true, 0, block.timestamp)) {
            ...
            // Verify if the function does not revert and handles TWAP correctly
            assertTrue(finalLiquidity != initialLiquidity, "Liquidity should have changed");
        } catch Error(string memory reason) {
            // Emit the reason for revert to help in debugging
            console2.log(reason);
